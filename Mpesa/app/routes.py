...
from .models import MpesaPayment, db
...
@mpesa.route('/access/token', methods=['GET'])
def get_access_token():
    token = MpesaAccessToken.get_access_token()
    return token

@mpesa.route('/online/lipa', methods=['POST'])
...
@mpesa.route('/c2b/register', methods=['POST'])
def register_urls():
    access_token = MpesaAccessToken.get_access_token()
...
@mpesa.route('/c2b/callback', methods=['POST'])
def callback():
    try:
