...
from app.utils.token import check_blacklist, token_required, generate_token, validate_input, verify_token
import logging
...
def logout():
    try:
        ...
        # Check if the token is already blacklisted (not strictly necessary since it should not be blacklisted here)
        ...
        print(f"Error occurred during logout: {str(e)}")
        return jsonify({'error': 'Something went wrong while logging out'}), 500
